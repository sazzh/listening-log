...
-- might be useful since no album preferences (add later if needed)
CREATE TABLE IF NOT EXISTS AlbumNotes (
	note_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
	user_id INTEGER NOT NULL REFERENCES users(user_id),
...
CREATE TRIGGER artistnotes_set_updated_at
BEFORE UPDATE ON artistnotes
FOR EACH ROW
EXECUTE FUNCTION set_updated_at();

CREATE TRIGGER songnotes_set_updated_at
BEFORE UPDATE ON songnotes
FOR EACH ROW
EXECUTE FUNCTION set_updated_at();

CREATE TRIGGER albumnotes_set_updated_at
BEFORE UPDATE ON albumnotes
FOR EACH ROW
EXECUTE FUNCTION set_updated_at();